...
def main(argv=None):
    ...
    snmp_client = SNMPClient(hostname=args.hostname, community=args.snmp_community, version=args.snmp_version)
    ...
    perf_data = {
        'cpu': f"{ap['cpu_usage']}%;{args.warning_cpu_threshold};{args.critical_cpu_threshold};0;100",
        'mem': f"{round(ap['mem_usage'], 1)}%;{args.warning_memory_threshold};{args.critical_memory_threshold};0;100",
    }
    ...
    print(f"{status_labels[status]} - {', '.join(errors + messages)} | {', '.join([f'{k}={v}' for k, v in perf_data.items()])}")
    print("\n".join(extra_lines))
